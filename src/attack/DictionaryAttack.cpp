...
DictionaryAttack::create(std::string_view path, unsigned thread_index, unsigned total_threads) {
    return create(path, thread_index, total_threads, 0);
}

std::expected<DictionaryAttack, CrackError> DictionaryAttack::create(std::string_view path,
    ...
    attack.resume_from_checkpoint(resume_offset);
...
std::size_t DictionaryAttack::checkpoint_position() const {
    return current_offset_;
}

void DictionaryAttack::resume_from_checkpoint(std::size_t absolute_offset) {
    current_offset_ = std::clamp(absolute_offset, start_offset_, end_offset_);
    words_read_ = count_lines_in_range(file_.data(), start_offset_, current_offset_);
    if (current_offset_ > start_offset_ && current_offset_ <= end_offset_ &&
        file_.data()[current_offset_ - 1] != '\n') {
        while (current_offset_ < end_offset_ && file_.data()[current_offset_] != '\n') {
            ++current_offset_;
        }
