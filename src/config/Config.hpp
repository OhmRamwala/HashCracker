...
Connects to:
  main.cpp              - CrackConfig populated from CLI args
...
constexpr int DEFAULT_CHECKPOINT_INTERVAL_SECONDS = 5;
...
constexpr std::size_t SHA3_256_HEX_LENGTH = 64;
constexpr std::size_t SHA3_512_HEX_LENGTH = 128;
...
struct CrackConfig {
    ...
    std::string hash_type_display = "Unknown";
    std::string hash_type_confidence = "specified";
    ...
    int checkpoint_interval_seconds = config::DEFAULT_CHECKPOINT_INTERVAL_SECONDS;
...
struct BatchCrackResult {
    ...
    std::size_t candidates_tested;
    double hashes_per_second;
    std::size_t cracked_count;
    std::size_t total_hashes;
    std::vector<BatchCrackItem> items;
};
