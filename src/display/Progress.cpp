...
Progress::Progress(std::string_view algorithm, std::string_view attack_mode,
                   std::string_view detection_mode, std::string_view confidence_label,
                   unsigned thread_count, std::size_t total_candidates, const std::atomic<bool> &found,
                   double initial_elapsed_seconds, const std::atomic<std::size_t> &tested)
    : algorithm_(algorithm), attack_mode_(attack_mode), detection_mode_(detection_mode),
      confidence_label_(confidence_label), thread_count_(thread_count), total_(total_candidates),
      found_(found), tested_(tested),
      start_time_(std::chrono::steady_clock::now() -
                  std::chrono::duration_cast<std::chrono::steady_clock::duration>(
                      std::chrono::duration<double>(initial_elapsed_seconds))) {}
...
void Progress::print_banner() const {
    ...
    auto confidence_color = [&]() -> std::string_view {
        if (confidence_label_ == "high") {
            return config::color::GREEN;
        }
        if (confidence_label_ == "low") {
            return config::color::YELLOW;
        }
        return config::color::CYAN;
    }();
    auto line3 = std::format("  {} {} ({}{}, {} confidence{}{})", config::box::VERTICAL,
                             algorithm_, confidence_color, detection_mode_, confidence_label_,
                             config::color::CYAN, config::color::RESET);
    ...
    std::println("{}{} {:<{}}{}{}", config::color::CYAN, config::box::VERTICAL, line2,
                 inner_width - 1, config::box::VERTICAL, config::color::RESET);
    std::println("{}{} {:<{}}{}{}", config::color::CYAN, config::box::VERTICAL, line3,
                 inner_width - 1, config::box::VERTICAL, config::color::RESET);
...
void Progress::print_batch_result(const BatchCrackResult &result) const {
    if (!is_tty()) {
        for (const auto &item : result.items) {
            if (item.cracked) {
                std::println("{}: {}", item.hash, item.plaintext);
            } else {
                std::println("{}: NOT FOUND", item.hash);
            }
        }
        return;
    }
    ...
    std::print("\033[3A\033[J");
    std::println("  {}{} BATCH COMPLETE {}{}{}", config::color::GREEN, config::symbol::CHECK,
                 std::string(24, '-'), config::color::RESET, "");
    std::println("  Algorithm: {}", result.algorithm);
    std::println("  Cracked:   {} / {}", result.cracked_count, result.total_hashes);
    std::println("  Speed:     {} {} {}", format_speed(result.hashes_per_second),
                 config::box::VERTICAL, format_time(result.elapsed_seconds));
    for (const auto &item : result.items) {
        if (item.cracked) {
            std::println("  {} {} {}{}", config::color::GREEN, item.hash, item.plaintext,
                         config::color::RESET);
        } else {
            std::println("  {} {} NOT FOUND{}", config::color::RED, item.hash,
                         config::color::RESET);
        }
    }
}
